...
const ChatboxContainer = styled.div<{ isCollapsed: boolean; isHidden: boolean }>`
  position: fixed;
  bottom: 20px;
  right: 20px;
  ...
  overflow: hidden;
  transform: translateX(${props => props.isHidden ? 'calc(100% + 20px)' : '0'});
...
const PracticeCode: React.FC = () => {
  ...
  const renderMessage = (content: string, isUser: boolean) => {
    if (isUser) {
      ...
      if (codeBlockMatch) {
        ...
        const afterCode = content.slice(content.indexOf(fullMatch) + fullMatch.length);
    ...
    const sanitizedContent = DOMPurify.sanitize(content, {
      ALLOWED_TAGS: ['div', 'p', 'br', 'strong', 'pre', 'code', 'ul', 'ol', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'],
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    animationFrameRef.current = requestAnimationFrame(() => {
      ...
      const editorContainer = document.querySelector('.monaco-editor')?.getBoundingClientRect();
                  ...
                  <div className="mt-4 border-l-4 border-blue-500 pl-4">
                    ...
                    {isHintVisible && (
                      <div className="mt-2 text-gray-300">
                        {exercise.hint || 'No hints available for this exercise.'}
                      </div>
                    )}
                  </div>
                </div>
                          ...
                          if (selection && !selection.isEmpty()) {
                            ...
                            if (selectedText.trim()) {
                              ...
                              if (editorContainer) {
                                ...
                                const selectionStartPos = editor.getScrolledVisiblePosition(selection.getStartPosition());
                                ...
                                if (selectionStartPos) {
                                  ...
                                  setTimeout(() => {
                                    setExplainButtonState({
                                      visible: true,
                                      x: containerRect.left + selectionStartPos.left + 10,
                                      y: containerRect.top + selectionStartPos.top - 40,
                    ...
                    <ConsoleContainer height={consoleHeight}>
                      ...
                      <ConsoleHeader>
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                          ...
                          <path d="M13.75 15.25H16.25" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                          <path d="M3.75 5.75C3.75 4.64543 4.64543 3.75 5.75 3.75H18.25C19.3546 3.75 20.25 4.64543 20.25 5.75V18.25C20.25 19.3546 19.3546 20.25 18.25 20.25H5.75C4.64543 20.25 3.75 19.3546 3.75 18.25V5.75Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        Console Output
                      </ConsoleHeader>
                      <div className={`flex-1 p-4 ${isError ? 'text-red-500' : hasWarnings ? 'text-yellow-500' : 'text-zinc-300'}`}>
          ...
          <>
            ...
            <ChatInput>
              <Input
                ...
                style={{ background: '#29334a', border: 'none', color: 'white' }}
              />
              <Button
                type="primary"
                icon={<SendOutlined />}
