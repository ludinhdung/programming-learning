...
import { CaretRightOutlined, ClockCircleOutlined, PlusCircleOutlined } from '@ant-design/icons';
...
const CourseDetail: FC = () => {
  ...
  const imgRef = useRef<HTMLImageElement | null>(null) as React.MutableRefObject<HTMLImageElement>;
  ...
  return (
    <div 
      ...
      style={{
        background: color && Array.isArray(color) 
          ? `linear-gradient(to bottom, rgb(${color.join(',')}),rgb(34, 78, 175))`
          : '#141924'
      }}>
      <Header />
        ...
        <div className="flex-1 bg-[#141924] px-32 ">
          ...
          <div className=" flex flex-col lg:flex-row lg:items-center justify-between mb-10">
            <div className="lg:w-3/5">
              <h1 className="text-5xl font-bold mb-6 text-primary-200 font-extrabold">{course.title.toUpperCase()}</h1>
              <p className="text-gray-300 leading-relaxed mb-6">{course.description}</p>
              <div className='flex items-center space-x-3 mb-6'>
                <button className='flex items-center text-md font-semibold bg-[#2a2e35] px-3 py-1.5  hover:bg-blue-700 transition-all duration-300 bg-blue-600 px-12 py-3'>
                  <span className=""><CaretRightOutlined className="text-sm mr-1"/></span>
                  Enroll Now
                </button>
                <button className='flex items-center text-md font-semibold bg-[#2a2e35] px-3 py-1.5  hover:bg-[#3a3e45] transition-all duration-300 px-12 py-3'>
                  <span className=""><PlusCircleOutlined className="text-sm mr-1 w-full"/></span>
                  Add to Wishlist
                </button>
              </div>
          ...
          {course.modules.map((module, index) => (
            <div key={module.id} className="mb-6">
              ...
              {module.videoUrl && (
                <div className="relative z-10 bg-[#1C2432] flex my-2 py-5 px-4 hover:bg-blue-800 transition-all duration-300 cursor-pointer">
                  <div className="my-2 w-[7%] max-w-[71px] max-h-[71px] rounded-full bg-[#191f29] text-2xl font-medium tracking-tight text-white transition-colors duration-300 border-6 border-[#13171c]">
                    <p className="text-center font-semibold text-2xl p-4">{index + 1}</p> 
                  </div>
                  <div className="module-item flex-1 px-5">
                    <p className="text-xl font-semibold text-gray-400 text-white mb-2">{module.title}</p>
                    ...
                    <div className='flex items-center gap-3 mt-3'>
                      <div className='flex gap-2'>
                        <ClockCircleOutlined />
                        <p className="text-xs text-gray-400 text-white font-medium">
                          {module.videoDuration ? formatDuration(module.videoDuration) : 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-400 text-white font-medium bg-[#1C2432] text-[#ef4444]">Video bài giảng</p>
                      </div>  
                    </div>      
                  </div>
                </div>
              ...
              {module.lessons.map((lesson, lessonIndex) => (
                <div key={lesson.id} className="relative z-10 bg-[#1C2432] flex my-2 py-5 px-4 hover:bg-red-800 transition-all duration-300 cursor-pointer">
                  <div className="my-2 w-[7%] max-w-[71px] max-h-[71px] rounded-full bg-[#191f29] text-2xl font-medium tracking-tight text-white transition-colors duration-300 border-6 border-[#13171c]">
                    <p className="text-center font-semibold text-2xl p-4">{lessonIndex + 1}</p> 
                  </div>
                  <div className="module-item flex-1 px-5">
                    <p className="text-xl font-semibold text-gray-400 text-white mb-2">{lesson.title}</p>
                    <p className="text-xs text-gray-400 text-white font-medium">
                      {lesson.description}
                    </p>
                    {lesson.type === 'video' && lesson.video && (
                      <div className='flex items-center gap-3 mt-3'>
                        ...
                        <div>
                          <p className="text-xs text-gray-400 text-white font-medium bg-[#1C2432] text-[#ef4444]">Free to watch</p>
                        </div>  
                      </div>
                    )}
                    {lesson.type === 'text' && (
                      <div className='flex items-center gap-3 mt-3'>
                        <div>
                          <p className="text-xs text-gray-400 text-white font-medium bg-[#1C2432] text-[#ef4444]">Bài đọc</p>
                        </div>  
                      </div>
                    )}
                    {lesson.type === 'coding' && lesson.coding && (
                      <div className='flex items-center gap-3 mt-3'>
                        <div>
                          <p className="text-xs text-gray-400 text-white font-medium bg-[#1C2432] text-[#3b82f6]">{lesson.coding.language}</p>
                        </div>  
                      </div>
                    )}
                    {lesson.type === 'test' && (
                      <div className='flex items-center gap-3 mt-3'>
                        <div>
                          <p className="text-xs text-gray-400 text-white font-medium bg-[#1C2432] text-[#10b981]">Bài kiểm tra</p>
                        </div>  
                      </div>
                    )}
                  </div>
      ...
      <div className="flex justify-center items-center text-center mx-[100px] bg-[#242C3A] py-[200px] px-[400px] my-10 text-white">
        ...
        <div className='w-1/2'>
          <div>
            <h3 className='mb-6 text-4xl font-medium leading-tighter text-left'>A massive community of programmers just like you.</h3>
            <p className="font-medium text-left"> Think of Laracasts sort of like Netflix, but for developers. You could spend weeks binging, and still not get through all the content we have to offer. </p>
          </div>
        </div>
      </div>
    </div>
