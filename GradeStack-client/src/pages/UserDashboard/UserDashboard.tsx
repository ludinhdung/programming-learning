...
// Import Collapse
import { Spin, message, Button, Avatar, Typography, Divider, Collapse } from 'antd';
import { BookOutlined, StarOutlined, HistoryOutlined, DeleteOutlined, EditOutlined, FileTextOutlined } from '@ant-design/icons';
import userService from '../../services/user.service';
import noteService from '../../services/note.service';
import type { EnrollmentRecord, BookmarkRecord } from '../../services/user.service';
// Import both interfaces
import type { Note, NotedCourseInfo, NotedCourseData } from '../../services/note.service';
        ...
        <div className="flex mb-6 bg-[#13151f] overflow-hidden">
            ...
            <div className="flex flex-col justify-between p-4 flex-grow">
                <div>
                    ...
                    {progress !== undefined && (
                        <div className="mt-2">
                            <div className="text-[#94a3b8] text-sm mb-1">Progress: {progress}%</div>
    ...
    return (
        <div className="flex justify-between items-center mb-6">
            ...
            {onClear && (
                <Button 
                    type="text" 
                    className="text-[#94a3b8] hover:text-white hover:bg-[#1e2736]"
                    onClick={onClear}
                >
                    Clear All
                </Button>
...
}> = ({ note, timestamp, courseTitle, onDelete }) => { // Added timestamp to destructuring
    return (
        <div className="flex items-center gap-4 p-4 bg-[#13151f] rounded-lg mb-4 hover:bg-[#1a1f2e] transition-colors">
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center text-[#94a3b8] text-sm">
                    <span className="truncate text-white flex-1"> {/* Made note content white and flexible */}
                        {note.content.length > 80 ? `${note.content.substring(0, 80)}...` : note.content}
                    </span>
                     {/* Keep date on the right */}
                     <span className="ml-2 whitespace-nowrap">{new Date(note.createdAt).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
...
const UserDashboard: FC = () => {
    ...
    const [activeCourseNotes, setActiveCourseNotes] = useState<Note[] | null>(null);
    ...
    const deleteNote = async (noteId: string) => {
        try {
            ...
            if (activeCourseNotes && typeof activePanelKey === 'string') { // Check if a single panel is active
                const currentCourseId = activePanelKey;
                // Find the course info to check if it's the active one
                const activeCourseInfo = notedCoursesInfo.find(info => info.id === currentCourseId);
                if (activeCourseInfo) {
                     setActiveCourseNotes(prevNotes => prevNotes ? prevNotes.filter(note => note.id !== noteId) : null);
    ...
    const handlePanelChange = async (key: string | string[] | undefined) => {
        ...
        if (typeof key === 'object' && key.length > 0) {
            ...
            try {
                ...
                const notes = await noteService.getNotesByCourse(courseId.at(0) as string);
        ...
        <div className="min-h-screen bg-[#0a1321]">
            ...
            <div className="container mx-auto px-6 py-8 max-w-5xl">
                <div className="mb-12">
                    ...
                    />
                    {loadingBookmarks && <div className="flex justify-center"><Spin size="large" /></div>}
                    {!loadingBookmarks && bookmarks.length === 0 && (
                        <p className="text-[#94a3b8] text-center py-6">You haven't bookmarked any courses yet.</p>
                    )}
                    {!loadingBookmarks && bookmarks.map((bookmark) => (
                        <CourseCard 
                            key={bookmark.id}
                ...
                <div className="mb-12">
                    ...
                    />
                    {loadingEnrolled && <div className="flex justify-center"><Spin size="large" /></div>}
                    {!loadingEnrolled && enrolledCourses.length === 0 && (
                        <p className="text-[#94a3b8] text-center py-6">You haven't viewed any courses yet.</p>
                    )}
                    {!loadingEnrolled && enrolledCourses.map((enrollment) => (
                        <CourseCard 
                ...
                <div>
                    ...
                    {loadingNotedCourses && <div className="flex justify-center py-6"><Spin size="large" /></div>}
                    ...
                    {!loadingNotedCourses && notedCoursesInfo.length > 0 && (
                        ...
                        >
                            {notedCoursesInfo.map((courseInfo) => (
                                <Panel
                                    header={<span className="text-white font-medium">{courseInfo.title}</span>}
                                    key={courseInfo.id}
                                ...
                                >
                                    ...
                                    {!loadingActiveNotes && activeCourseNotes && activeCourseNotes.length > 0 && (
                                        <div>
                                    ...
                                    {!loadingActiveNotes && activeCourseNotes && activeCourseNotes.length === 0 && (
                                        <p className="text-[#94a3b8] text-center py-4">No notes found for this course.</p>
                                    )}
                                </Panel>
                            ))}
                        </Collapse>
