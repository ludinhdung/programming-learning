...
import { CreateWorkshopDto } from '../../../types/workshop.types';
...
/**
 * Trang tạo workshop mới cho giảng viên
 */
const CreateWorkshopPage = () => {
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (!instructorId) {
        message.error('Không thể xác định giảng viên');
        return;
      }
      ...
      const response = await workshopService.createWorkshop(instructorId, workshopData);
      console.log('Workshop created successfully:', response);
      
      message.success('Tạo workshop thành công');
      navigate('/instructor-lead-management/workshops');
    } catch (error) {
      console.error('Lỗi khi tạo workshop:', error);
      message.error('Không thể tạo workshop. Vui lòng thử lại sau.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-6 instructor-lead-bg min-h-screen">
      <div className="mb-6">
        <div className="flex text-3xl justify-start uppercase font-extrabold">
          ...
          <span className="text-white">Tạo Workshop Mới</span>
        </div>
      </div>

      <Card className="instructor-lead-card overflow-hidden">
        <div className="border-l-4 border-blue-600 pl-4 mb-6">
          <Title level={4} className="instructor-lead-title m-0">Thông tin Workshop</Title>
          <p className="instructor-lead-text-secondary mt-1">Nhập thông tin chi tiết về workshop</p>
        </div>
        <Form
          form={form}
          layout="vertical"
            ...
            <div>
              ...
              {isOnline ? (
                <Form.Item
                  ...
                  tooltip="Nhập link Google Meet hoặc chỉ nhập mã phòng học (ví dụ: abc-defg-hij)"
                >
                  <Input 
                    placeholder="Ví dụ: https://meet.google.com/abc-defg-hij hoặc chỉ nhập abc-defg-hij" 
                    className="instructor-lead-input"
                    prefix={<span style={{ color: '#4285F4' }}>meet.google.com/</span>}
                  />
                </Form.Item>
              ) : (
                <Form.Item
                    ...
                    {fields.map(({ key, name, ...restField }) => (
                      <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                        <Form.Item
                          ...
                          name={name}
                          className="w-full"
                          rules={[{ required: true, message: 'Vui lòng nhập tài liệu hoặc xóa trường này' }]}
                          tooltip="Nhập URL tài liệu hoặc tên tài liệu. Nếu là URL, học viên có thể truy cập trực tiếp."
                        >
                          <Input 
                            placeholder="Tên tài liệu hoặc URL (ví dụ: https://docs.google.com/...)" 
                            className="instructor-lead-input"
                          />
                        </Form.Item>
                        {fields.length > 1 && (
                    ...
                    {fields.map(({ key, name, ...restField }) => (
                      <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                        <Form.Item
                          ...
                          name={name}
                          className="w-full"
                          tooltip="Nhập từ khóa để phân loại workshop (ví dụ: React, JavaScript, API)"
                        >
                          <Input 
                            placeholder="Tag (ví dụ: React, JavaScript, API)" 
                            className="instructor-lead-input"
                          />
                        </Form.Item>
                        {fields.length > 1 && (
